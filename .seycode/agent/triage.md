...
**Only** add if it's specifically about the Desktop application or `seycode web` view. **Do not** add for terminal, TUI, or general seycode issues.
...
**Only** add if the issue mentions "zen" or "seycode zen" or "seycode black".

If the issue doesn't have "zen" or "seycode black" in it then don't add zen label

#### core

Use for core server issues in `packages/seycode/`, excluding `packages/seycode/src/cli/cmd/tui/`.
...
TUI (`packages/seycode/src/cli/cmd/tui/...`):
...
Core (`packages/seycode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for seycode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
