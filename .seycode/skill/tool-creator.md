...
2. **Create the tool file** in `packages/seycode/src/tool/builtin/your-tool.ts`:
   ```typescript
   import { z } from "zod"
...
3. **Register the tool** in `packages/seycode/src/tool/registry.ts`:
   - Import your tool
...
- `packages/seycode/src/tool/builtin/read.ts` - File reading
- `packages/seycode/src/tool/builtin/bash.ts` - Command execution
- `packages/seycode/src/tool/builtin/glob.ts` - Pattern matching
