...
# Package manageri
npm i -g seycode-ai@latest        # ili bun/pnpm/yarn
scoop install seycode             # Windows
choco install seycode             # Windows
brew install anomalyco/tap/seycode # macOS i Linux (preporučeno, uvijek ažurno)
brew install seycode              # macOS i Linux (zvanična brew formula, rjeđe se ažurira)
sudo pacman -S seycode            # Arch Linux (Stable)
paru -S seycode-bin               # Arch Linux (Latest from AUR)
mise use -g seycode               # Bilo koji OS
nix run nixpkgs#seycode           # ili github:anomalyco/seycode za najnoviji dev branch
```
...
OpenCode je dostupan i kao desktop aplikacija. Preuzmi je direktno sa [stranice izdanja](https://github.com/anomalyco/seycode/releases) ili sa [seycode.ai/download](https://seycode.ai/download).
...
4. `$HOME/.seycode/bin` - Podrazumijevana rezervna lokacija
...
Saznaj više o [agentima](https://seycode.ai/docs/agents).

### Dokumentacija

Za više informacija o konfiguraciji OpenCode-a, [**pogledaj dokumentaciju**](https://seycode.ai/docs).
...
Ako radiš na projektu koji je povezan s OpenCode-om i koristi "seycode" kao dio naziva, npr. "seycode-dashboard" ili "seycode-mobile", dodaj napomenu u svoj README da projekat nije napravio OpenCode tim i da nije povezan s nama.
...
- 100% open source
- Nije vezan za jednog provajdera. Iako preporučujemo modele koje nudimo kroz [OpenCode Zen](https://seycode.ai/zen), OpenCode možeš koristiti s Claude, OpenAI, Google ili čak lokalnim modelima. Kako modeli napreduju, razlike među njima će se smanjivati, a cijene padati, zato je nezavisnost od provajdera važna.
...
**Pridruži se našoj zajednici** [Discord](https://discord.gg/seycode) | [X.com](https://x.com/seycode)
