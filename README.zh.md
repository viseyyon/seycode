...
brew install anomalyco/tap/seycode # macOS 和 Linux（推荐，始终保持最新）
brew install seycode              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S seycode            # Arch Linux (Stable)
paru -S seycode-bin               # Arch Linux (Latest from AUR)
mise use -g seycode               # 任意系统
nix run nixpkgs#seycode           # 或用 github:anomalyco/seycode 获取最新 dev 分支
```
...
OpenCode 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/seycode/releases) 或 [seycode.ai/download](https://seycode.ai/download) 下载。
...
了解更多 [Agents](https://seycode.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://seycode.ai/docs)。
...
如果你在项目名中使用了 “seycode”（如 “seycode-dashboard” 或 “seycode-mobile”），请在 README 里注明该项目不是 OpenCode 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [OpenCode Zen](https://seycode.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
...
**加入我们的社区** [Discord](https://discord.gg/seycode) | [X.com](https://x.com/seycode)
