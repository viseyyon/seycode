...
brew install anomalyco/tap/seycode # macOS e Linux (recomendado, sempre atualizado)
brew install seycode              # macOS e Linux (fórmula oficial do brew, atualiza menos)
sudo pacman -S seycode            # Arch Linux (Stable)
paru -S seycode-bin               # Arch Linux (Latest from AUR)
mise use -g seycode               # qualquer sistema
nix run nixpkgs#seycode           # ou github:anomalyco/seycode para a branch dev mais recente
```
...
O OpenCode também está disponível como aplicativo desktop. Baixe diretamente pela [página de releases](https://github.com/anomalyco/seycode/releases) ou em [seycode.ai/download](https://seycode.ai/download).
...
4. `$HOME/.seycode/bin` - Fallback padrão
...
Saiba mais sobre [agents](https://seycode.ai/docs/agents).

### Documentação

Para mais informações sobre como configurar o OpenCode, [**veja nossa documentação**](https://seycode.ai/docs).
...
Se você estiver trabalhando em um projeto relacionado ao OpenCode e estiver usando "seycode" como parte do nome (por exemplo, "seycode-dashboard" ou "seycode-mobile"), adicione uma nota no README para deixar claro que não foi construído pela equipe do OpenCode e não é afiliado a nós de nenhuma forma.
...
**Junte-se à nossa comunidade** [Discord](https://discord.gg/seycode) | [X.com](https://x.com/seycode)
