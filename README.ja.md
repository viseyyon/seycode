...
# パッケージマネージャー
npm i -g seycode-ai@latest        # bun/pnpm/yarn でもOK
scoop install seycode             # Windows
choco install seycode             # Windows
brew install anomalyco/tap/seycode # macOS と Linux（推奨。常に最新）
brew install seycode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S seycode            # Arch Linux (Stable)
paru -S seycode-bin               # Arch Linux (Latest from AUR)
mise use -g seycode               # どのOSでも
nix run nixpkgs#seycode           # または github:anomalyco/seycode で最新 dev ブランチ
```
...
OpenCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/seycode/releases) から直接ダウンロードするか、[seycode.ai/download](https://seycode.ai/download) を利用してください。
...
4. `$HOME/.seycode/bin` - デフォルトのフォールバック
...
[agents](https://seycode.ai/docs/agents) の詳細はこちら。

### ドキュメント

OpenCode の設定については [**ドキュメント**](https://seycode.ai/docs) を参照してください。
...
OpenCode に関連するプロジェクトで、名前に "seycode"（例: "seycode-dashboard" や "seycode-mobile"）を含める場合は、そのプロジェクトが OpenCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[OpenCode Zen](https://seycode.ai/zen) で提供しているモデルを推奨しますが、OpenCode は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
...
**コミュニティに参加** [Discord](https://discord.gg/seycode) | [X.com](https://x.com/seycode)
