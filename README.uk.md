...
# Менеджери пакетів
npm i -g seycode-ai@latest        # або bun/pnpm/yarn
scoop install seycode             # Windows
choco install seycode             # Windows
brew install anomalyco/tap/seycode # macOS і Linux (рекомендовано, завжди актуально)
brew install seycode              # macOS і Linux (офіційна формула Homebrew, оновлюється рідше)
sudo pacman -S seycode            # Arch Linux (Stable)
paru -S seycode-bin               # Arch Linux (Latest from AUR)
mise use -g seycode               # Будь-яка ОС
nix run nixpkgs#seycode           # або github:anomalyco/seycode для найновішої dev-гілки
```
...
OpenCode також доступний як десктопний застосунок. Завантажуйте напряму зі [сторінки релізів](https://github.com/anomalyco/seycode/releases) або [seycode.ai/download](https://seycode.ai/download).
...
4. `$HOME/.seycode/bin` - Резервний варіант за замовчуванням
...
Дізнайтеся більше про [agents](https://seycode.ai/docs/agents).

### Документація

Щоб дізнатися більше про налаштування OpenCode, [**перейдіть до нашої документації**](https://seycode.ai/docs).
...
Якщо ви працюєте над проєктом, пов'язаним з OpenCode, і використовуєте "seycode" у назві, наприклад "seycode-dashboard" або "seycode-mobile", додайте примітку до свого README.
...
**Приєднуйтеся до нашої спільноти** [Discord](https://discord.gg/seycode) | [X.com](https://x.com/seycode)
