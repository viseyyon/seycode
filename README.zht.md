...
# 套件管理員
npm i -g seycode-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install seycode             # Windows
choco install seycode             # Windows
brew install anomalyco/tap/seycode # macOS 與 Linux（推薦，始終保持最新）
brew install seycode              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S seycode            # Arch Linux (Stable)
paru -S seycode-bin               # Arch Linux (Latest from AUR)
mise use -g seycode               # 任何作業系統
nix run nixpkgs#seycode           # 或使用 github:anomalyco/seycode 以取得最新開發分支
```
...
OpenCode 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/anomalyco/seycode/releases) 或 [seycode.ai/download](https://seycode.ai/download) 下載。
...
了解更多關於 [Agents](https://seycode.ai/docs/agents) 的資訊。

### 線上文件

關於如何設定 OpenCode 的詳細資訊，請參閱我們的 [**官方文件**](https://seycode.ai/docs)。
...
如果您正在開發與 OpenCode 相關的專案，並在名稱中使用了 "seycode"（例如 "seycode-dashboard" 或 "seycode-mobile"），請在您的 README 中加入聲明，說明該專案並非由 OpenCode 團隊開發，且與我們沒有任何隸屬關係。
...
- 100% 開源。
- 不綁定特定的服務提供商。雖然我們推薦使用透過 [OpenCode Zen](https://seycode.ai/zen) 提供的模型，但 OpenCode 也可搭配 Claude, OpenAI, Google 甚至本地模型使用。隨著模型不斷演進，彼此間的差距會縮小且價格會下降，因此具備「不限廠商 (provider-agnostic)」的特性至關重要。
- 內建 LSP (語言伺服器協定) 支援。
...
**加入我們的社群** [Discord](https://discord.gg/seycode) | [X.com](https://x.com/seycode)
