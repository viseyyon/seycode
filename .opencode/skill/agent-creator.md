...
1. **Define agent configuration** in project's `opencode.json`:
   ```json
   {
     "agents": {
       "your-agent": {
...
1. Load config: Restart SeyCode or use `opencode attach`
...
See `packages/opencode/src/agent/agent.ts` for agent schema and implementation.
