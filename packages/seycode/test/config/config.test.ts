...
test("loads config from .seycode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".seycode")
...
test("loads agents from .seycode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".seycode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .seycode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".seycode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .seycode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".seycode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .seycode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".seycode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .seycode config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .seycode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("local .seycode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .seycode directory config enables it
      const opencodeDir = path.join(dir, ".seycode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "opencode.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.seycode/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.seycode/plugin/oh-my-opencode.js")
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .seycode/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .seycode directory with a command
          const opencodeDir = path.join(dir, ".seycode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .seycode should NOT be in directories list
